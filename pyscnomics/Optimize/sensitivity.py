...
def adjust_contract(contract: CostRecovery | GrossSplit,
    ...
    if adjusted_variable == 'CAPEX':
        ...
        contract.tangible_cost = tuple(list_capex)
    ...
    if adjusted_variable == 'Oil Price':
        ...
        contract.lifting = tuple(list_lifting)

    # Modifying the Lifting Rate if the chosen adjusted variable is Prod Factor
    if adjusted_variable == 'Prod Factor':
        list_lifting = []
        for lft in contract.lifting:
            lift = lft * multiply_factor
            list_lifting.append(lift)

        contract.lifting = tuple(list_lifting)

    contract_adjusted = make_new_contract(contract=contract)
    contract_adjusted.run(**contract_arguments)
    summary_arguments['contract'] = contract_adjusted
...
def sensitivity_psc(steps: int,
    ...
    steps_arr = np.concatenate((np.array([1]), left_steps, right_steps))

    # Contract attributes that have to be adjusted, they are OPEX, CAPEX, Oil Price, Gas Price, Prod Factor
    ...
    for i in steps_arr:
        ...
        result_psc = adjust_contract(contract=contract,
                                     adjusted_variable='Oil Price',
                                     multiply_factor=i,
                                     contract_arguments=contract_arguments,
                                     summary_arguments=summary_arguments)

        # Appending the contract to result list
        result_npv.append(result_psc['ctr_npv'])
        result_irr.append(result_psc['ctr_irr'])
        result_pi.append(result_psc['ctr_pi'])
        result_gov_take.append(result_psc['gov_take'])
        result_ctr_take.append(result_psc['ctr_net_share'])

    # Adding the ability to retrieve the summary information as needed
    # NPV, IRR, P/I, Government Take, Contractor Take
