...
def get_summary(
    ...
    if np.sum(contract._oil_lifting.get_lifting_rate_ghv_arr()) == 0:
        ...
        oil_wap = np.divide(
            np.sum(contract._oil_revenue),
            np.sum(contract._oil_lifting.get_lifting_rate_ghv_arr()),
    ...
    if np.sum(contract._gas_lifting.get_lifting_rate_ghv_arr()) == 0:
        ...
        gas_wap = np.divide(
            np.sum(
                ...
                * contract._gas_lifting.get_ghv_arr()
            ),
            np.sum(
                contract._gas_lifting.get_lifting_rate_ghv_arr()
                * contract._gas_lifting.get_ghv_arr()
    ...
    total_carry_forward_depreciation = (
        oil_carry_forward_depreciation + gas_carry_forward_depreciation
    )
    ...
    # Capex
    oil_capex = np.sum(contract._oil_capital_expenditures_post_tax)
    gas_capex = np.sum(contract._gas_capital_expenditures_post_tax)

    # OPEX and ASR (Non-Capital Cost)
    opex = np.sum(
        contract._oil_opex_expenditures_post_tax
        + contract._gas_opex_expenditures_post_tax,
        dtype=float,
    )
    asr = np.sum(
        contract._oil_asr_expenditures_post_tax
        + contract._gas_asr_expenditures_post_tax,
        dtype=float,
    )
    lbt = np.sum(
        contract._oil_lbt_expenditures_post_tax
        + contract._gas_lbt_expenditures_post_tax,
        dtype=float,
    )
    ...
    years_sunk_cost_pooled = contract.project_years[
        (len(contract.project_years) - len(cashflow_sunk_cost_pooled)) :
    ]

    if (
        isinstance(contract, CostRecovery)
        or isinstance(contract, GrossSplit)
        or isinstance(contract, Transition)
    ):
        # Government DDMO
    ...
    if npv_mode == NPVSelection.NPV_SKK_REAL_TERMS:
        ...
        ctr_npv = npv_skk_real_terms(
            cashflow=contract._consolidated_cashflow,
        ...
        gov_take_npv = npv_skk_real_terms(
            cashflow=contract._consolidated_government_take,
            cashflow_years=contract.project_years,
            discount_rate=discount_rate,
            reference_year=reference_year,
            discounting_mode=discounting_mode,
    ...
    # NPV Calculation for Point Forwards
        ...
        ref_year_arr = np.full_like(
            contract._consolidated_cashflow, fill_value=reference_year
        )
        cashflow_point_forward = np.where(
            contract.project_years >= ref_year_arr, contract._consolidated_cashflow, 0
        )
        gross_revenue_point_forward = np.where(
            contract.project_years >= ref_year_arr, contract._consolidated_revenue, 0
        )
    ...
    if isinstance(contract, CostRecovery):
        # Cost Recovery
        cost_recovery = np.sum(
            contract._consolidated_cost_recovery_after_tf, dtype=float
        )
    ...
    if isinstance(contract, GrossSplit):
        ...
        carry_forcost_over_deductible_cost = np.divide(
            carry_forward_deductible_cost, deductible_cost, where=deductible_cost != 0
        )

        # CTR Gross Share
        ctr_gross_share = np.sum(
            contract._consolidated_ctr_share_before_tf, dtype=float
        )

        # GOV GOV Gross Share
        gov_gross_share = np.sum(
            contract._consolidated_gov_share_before_tf, dtype=float
        )
    ...
    if isinstance(contract, Transition):
        ...
        deductible_cost = np.sum(
            contract._oil_deductible_cost + contract._gas_deductible_cost, dtype=float
        )
        ...
        ctr_equity_share = np.sum(
            contract._oil_ctr_ets + contract._gas_ctr_ets, dtype=float
        )
        gov_equity_share = np.sum(
            contract._oil_gov_ets + contract._gas_gov_ets, dtype=float
        )
        ...
        ctr_net_operating_profit_over_grossrev = (
            ctr_net_operating_profit / gross_revenue
        )
...
class Summary:
    ...
    def _get_sum_lifting(self):
        ...
        """
        # Lifting Oil
        self.lifting_oil = float(
            np.sum(self.contract._oil_lifting.get_lifting_rate_ghv_arr(), dtype=float)
        )
        if np.sum(self.contract._oil_lifting.get_lifting_rate_ghv_arr()) == 0:
            self.oil_wap = 0.0
        else:
            self.oil_wap = np.divide(
                np.sum(self.contract._oil_revenue),
                np.sum(self.contract._oil_lifting.get_lifting_rate_ghv_arr()),
                where=np.sum(self.contract._oil_lifting.get_lifting_rate_ghv_arr())
                != 0,
            )

        self.lifting_gas = float(
            np.sum(self.contract._gas_lifting.get_lifting_rate_ghv_arr(), dtype=float)
        )
        ...
        if np.sum(self.contract._gas_lifting.get_lifting_rate_ghv_arr()) == 0:
            ...
            self.gas_wap = np.divide(
                np.sum(
                    self.contract._gas_wap_price
                    * self.contract._gas_lifting.get_lifting_rate_ghv_arr()
                ),
                np.sum(self.contract._gas_lifting.get_lifting_rate_ghv_arr()),
                where=np.sum(self.contract._gas_lifting.get_lifting_rate_ghv_arr())
                != 0,
            )
    ...
    def _get_gross_revenue(self):
        ...
        self.gross_revenue = float(
            np.sum(self.gross_revenue_oil + self.gross_revenue_gas, dtype=float)
        )
    ...
    def _get_sunk_cost(self):
        ...
        """
        # Sunk Cost
        self.sunk_cost = float(
            np.sum(
                self.contract._oil_sunk_cost + self.contract._gas_sunk_cost, dtype=float
            )
        )
        self.sunk_cost_extended = np.concatenate(
            (
                ...
                np.zeros(
                    len(self.contract.project_years)
                    - len(self.contract._consolidated_sunk_cost)
                ),
            )
        )
    ...
    def _get_intangible_cost(self):
        ...
        self.intangible = float(
            np.sum(
                self.contract._oil_intangible_expenditures_post_tax
                + self.contract._gas_intangible_expenditures_post_tax
    ...
    def _get_capex(self):
        ...
        """
        # Capex
        self.oil_capex = float(np.sum(self.contract._oil_capital_expenditures_post_tax))
        self.gas_capex = float(np.sum(self.contract._gas_capital_expenditures_post_tax))
    ...
    def _get_asr(self):
        ...
        self.asr = float(
            np.sum(
                self.contract._oil_asr_expenditures_post_tax
                + self.contract._gas_asr_expenditures_post_tax,
                dtype=float,
            )
        )
    ...
    def _get_cashflow_sunkcost_pooled(self):
        ...
        # Years sunk cost pooled
        self.years_sunk_cost_pooled = self.contract.project_years[
            (len(self.contract.project_years) - len(self.cashflow_sunk_cost_pooled)) :
        ]
    ...
    def _get_ddmo(self):
        ...
        # Government DDMO
        if (
            isinstance(self.contract, CostRecovery)
            or isinstance(self.contract, GrossSplit)
            or isinstance(self.contract, Transition)
        ):
    ...
    def _npv_pointforward_terms(self):
        ...
        #  Modifying the Contractor Net Cashflow since the cashflow before the reference year is neglected.
        ref_year_arr = np.full_like(
            self.contract._consolidated_cashflow, fill_value=self.reference_year
        )
        ...
        gross_revenue_point_forward = np.sum(gross_revenue_point_forward, dtype=float)
        self.ctr_net_cashflow_over_gross_rev = (
            self.ctr_net_cashflow / gross_revenue_point_forward
        )
    ...
    def _get_profitability_index(self):
        ...
        if self.profitability_discounted:
            self.ctr_pv_ratio = np.divide(
                self.ctr_npv, self.investment_npv, where=self.investment_npv != 0
            )
        ...
        # Condition when the Profitability Index is calculated using un-discounted investment
            ...
            self.ctr_pv_ratio = np.divide(
                self.ctr_npv, self.investment_pi, where=self.investment_pi != 0
            )
    ...
    def _get_costrec_deductible_cost(self):
        ...
        if isinstance(self.contract, CostRecovery):
            # Cost Recovery
            self.cost_recovery = np.sum(
                self.contract._consolidated_cost_recovery_after_tf, dtype=float
            )
            ...
            self.deductible_cost_over_gross_rev = (
                self.deductible_cost / self.gross_revenue
            )
        else:
            pass
    ...
    def _get_unrecoverable_carryforward_cost(self):
        ...
        if isinstance(self.contract, CostRecovery):
            ...
            # Carry Forward Cost
            self.carry_forward_deductible_cost = (
                self.contract._consolidated_carward_deduct_cost[-1]
            )
            self.carry_forcost_over_gross_share = (
                self.carry_forward_deductible_cost / self.gross_revenue
            )
            self.carry_forcost_over_deductible_cost = (
                self.carry_forward_deductible_cost / self.deductible_cost
            )
    ...
    def _get_gross_share(self):
        ...
        if isinstance(self.contract, CostRecovery):
            ...
            self.ctr_gross_share = float(
                np.sum(self.contract._consolidated_contractor_share, dtype=float)
            )
            self.gov_gross_share = float(
                np.sum(self.contract._consolidated_government_share, dtype=float)
            )

        elif isinstance(self.contract, GrossSplit):
            # Gross Split Gross Share
            self.ctr_gross_share = float(
                np.sum(self.contract._consolidated_ctr_share_before_tf, dtype=float)
            )
            self.gov_gross_share = float(
                np.sum(self.contract._consolidated_gov_share_before_tf, dtype=float)
            )
    ...
    def _get_net_share(self):
        ...
        self.ctr_net_share = float(
            np.sum(self.contract._consolidated_ctr_net_share, dtype=float)
        )
    ...
    def _get_netcashfflow(self):
        ...
        self.ctr_net_cashflow_over_gross_rev = (
            self.ctr_net_cashflow / self.gross_revenue
        )
    ...
    def _get_equity_share(self):
        ...
        """
        self.gov_equity_share = float(
            np.sum(self.contract._consolidated_government_share)
        )
    ...
    def _get_transition_indicator(self):
        ...
        self.deductible_cost = np.sum(
            self.contract._oil_deductible_cost + self.contract._gas_deductible_cost,
            dtype=float,
        )
        self.deductible_cost_over_grossrev = self.deductible_cost / self.gross_revenue

        self.carry_forward_cost = (
            self.contract._oil_unrec_cost + self.contract._gas_unrec_cost
        )[-1]
        self.carry_forward_cost_over_grossrev = (
            self.carry_forward_cost / self.gross_revenue
        )

        self.ctr_equity_share = float(
            np.sum(self.contract._oil_ctr_ets + self.contract._gas_ctr_ets, dtype=float)
        )
        self.gov_equity_share = float(
            np.sum(self.contract._oil_gov_ets + self.contract._gas_gov_ets, dtype=float)
        )
        self.equity_to_be_split = self.ctr_equity_share + self.gov_equity_share

        self.ctr_net_operating_profit = np.sum(
            self.contract._net_operating_profit, dtype=float
        )
        self.ctr_net_operating_profit_over_grossrev = (
            self.ctr_net_operating_profit / self.gross_revenue
        )

        self.ctr_net_cash_flow = np.sum(self.contract._ctr_net_cashflow, dtype=float)
        self.ctr_net_cash_flow_over_grossrev = (
            self.ctr_net_cash_flow / self.gross_revenue
        )
    ...
    def run(self):
        ...
        if isinstance(self.contract, CostRecovery) or isinstance(
            self.contract, GrossSplit
        ):
