...
def _get_multipliers(
    ...
    # Previous approach
    # -----------------
    # lower_mults = np.linspace(min_val, base_value, step + 1).round(decimals=2)
    # upper_mults = np.linspace(base_value, max_val, step + 1).round(decimals=2)
...
def _prepare_adjusted_parameters_single_contract(
    ...
    def _adjust_lifting(fluid_type: FluidType, target_field: str):
        ...
        if target_field not in ["lifting_rate", "price"]:
            raise SensitivityException(
                f"Unrecognized target_field: {target_field}. Parameter target_field "
                f"must be 'lifting_rate' or 'price'."
            )
    ...
    # Adjustment to OPEX cost, modify the `fixed cost` attribute of class OPEX
    elif element == "OPEX":
        opex_adjusted = tuple(
            [
                OPEX(
...
def sensitivity_psc(
    ...
    """
    Perform a one-factor-at-a-time sensitivity analysis on a PSC contract.

    The function adjusts a set of predefined contract factors (CAPEX, OPEX,
    OILPRICE, GASPRICE, OILLIFTING, GASLIFTING) by a sequence of multipliers
    (generated by `_get_multipliers`) and runs the contract for each adjusted
    value. For every run it collects the contract summary via
    `contract.get_summary(**summary_arguments)`, then reorganizes those summaries
    into sensitivity results for a fixed list of economic indicators
    (ctr_npv, ctr_irr, ctr_pi, ctr_pot, gov_take, ctr_net_share).

    Parameters
    ----------
    contract : BaseProject | CostRecovery | GrossSplit | Transition
        A contract object implementing the project interface. The object is
        expected to accept adjustments via `_adjust_contract(..., run_contract=True)`
        and to expose a `.get_summary(**summary_arguments)` method that returns
        a mapping of summary indicators.
    contract_arguments : dict
        Arguments forwarded to `_adjust_contract` / contract constructor as needed.
        Typically contains the base project input data required by `_adjust_contract`.
    summary_arguments : dict
        Keyword arguments forwarded to each `contract.get_summary(...)` call.
        Example keys might include `summary_result=True` or `year_range=(start, end)`,
        depending on the project's API.
    min_deviation : float
        Minimum multiplier (or relative change) used to generate sensitivity
        multipliers (e.g., 0.8 for -20%).
    max_deviation : float
        Maximum multiplier (or relative change) used to generate sensitivity
        multipliers (e.g., 1.2 for +20%).
    base_value : float, optional
        The base multiplier (default is 1.0). Passed to `_get_multipliers`.
    step : int, optional
        Number of steps (or number of multipliers) between `min_deviation` and
        `max_deviation` (default is 10). Passed to `_get_multipliers`.
    dataframe_output : bool, optional
        If True (default) return sensitivity results as a dict of pandas
        DataFrames (one DataFrame per indicator), otherwise return a nested
        dictionary structure.

    Notes
    -----
    - The function relies on two helpers: `_get_multipliers(...)` which must
      return an iterable of numeric multipliers, and `_adjust_contract(...)`
      which must return a contract-like object whose `.get_summary(...)` returns
      a mapping with keys including the indicators listed below.
    - The function collects these indicators:
      ["ctr_npv", "ctr_irr", "ctr_pi", "ctr_pot", "gov_take", "ctr_net_share"].
      If a summary does not include an indicator, `None` is stored for that
      element/multiplier combination.
    - Execution may be time-consuming because `_adjust_contract(..., run_contract=True)`
      appears to re-run the contract model for every multiplier and every factor.
    - Dictionary insertion/iteration order (Python 3.7+) determines the order of
      multipliers used when building DataFrames; `_get_multipliers` should
      therefore return multipliers in the intended sequence (e.g., ascending).
    """
    ...
    elements = ["CAPEX", "OPEX", "OILPRICE", "GASPRICE", "OILLIFTING", "GASLIFTING"]
    ...
    # Retrieve NPV, IRR, PI, POT, Gov Take, and Contractor Share
    indicator_list = [
        "ctr_npv",
        "ctr_irr",
        "ctr_pi",
