...
class MonteCarloData:
    ...
    # def __post_init__(
    #     self,
    #     min_values_init: np.ndarray,
    #     max_values_init: np.ndarray,
    #     mean_values_init: np.ndarray,
    #     std_dev_init: np.ndarray,
    # ):
    #     # Specify conditions for empty data
    #     if (
    #         all([pd.isna(i) for i in min_values_init])
    #         and all([pd.isna(j) for j in max_values_init])
    #         and all([pd.isna(k) for k in std_dev_init])
    #     ):
    ...
    #         if pd.isna(self.run_number):
    #             self.run_number = int(100)
    #
    #         # Prepare attribute parameter
    #         if not isinstance(self.parameter, list):
    #             raise MonteCarloDataException(
    #                 f"Attribute parameter must be given in the form of list, "
    #                 f"not {self.parameter.__class__.__qualname__}."
    #             )
    #
    #         # Prepare attribute distribution
    #         if not isinstance(self.distribution, list):
    #             raise MonteCarloDataException(
    #                 f"Attribute distribution must be given in the form of list, "
    #                 f"not {self.distribution.__class__.__qualname__}."
    #             )
    #
    #         # Prepare attribute min_values
    #         if not isinstance(min_values_init, np.ndarray):
    #             raise MonteCarloDataException(
    #                 f"Attribute min_values_init must be given in the form of np.ndarray, "
    #                 f"not {min_values_init.__class__.__qualname__}."
    #             )
    #
    #         min_values_nan = np.argwhere(pd.isna(min_values_init)).ravel()
    #         if len(min_values_nan) > 0:
    #             raise MonteCarloDataException(
    #                 f"Attribute min_values is missing a values(s). "
    #                 f"Please check the input data for min_values."
    #             )
    #
    #         self.min_values = min_values_init.copy()
    #
    #         # Prepare attribute max_values
    #         if not isinstance(max_values_init, np.ndarray):
    #             raise MonteCarloDataException(
    #                 f"Attribute max_values_init must be given in the form of np.ndarray, "
    #                 f"not {max_values_init.__class__.__qualname__}."
    #             )
    #
    #         max_values_nan = np.argwhere(pd.isna(max_values_init)).ravel()
    #         if len(max_values_nan) > 0:
    #             raise MonteCarloDataException(
    #                 f"Attribute max_values is missing a values(s). "
    #                 f"Please check the input data for max_values."
    #             )
    #
    #         self.max_values = max_values_init.copy()
    #
    #         # Prepare attribute standard deviation
    #         if not isinstance(std_dev_init, np.ndarray):
    #             raise MonteCarloDataException(
    #                 f"Attribute std_dev_init must be given in the form of np.ndarray, "
    #                 f"not {std_dev_init.__class__.__qualname__}."
    #             )

            # std_dev_nan = np.argwhere(pd.isna(std_dev_init)).ravel()
            # if len(std_dev_nan) > 0:
            #     for i in std_dev_nan:
            #         if self.distribution[i] == "Uniform":
            #             std_dev_init[i] = 0.0
            #         elif self.distribution[i] == "Triangular":
            #             pass
            #         elif self
            #
            # print('\t')
            # print(f'Filetype: {type(std_dev_nan)}')
            # print('std_dev_nan = \n', std_dev_nan)
            #
            # print('\t')
            # print(f'Filetype: {type(std_dev_init)}')
            # print('std_dev_init = \n', std_dev_init)
        ...
        #     )
        #
        # min_values_nan = np.argwpd.isna(self.min_values)
        #
